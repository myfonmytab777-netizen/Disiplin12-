const STORAGE_KEY = 'AURA_STREAK_HABITS_DATA_V24';
...
// RANAH KULTIVASI (SWALLOWED STAR) 19 LEVEL
function getCultivationRealm(streak) {
  if (streak >= 730) return { title: 'OverLord', bg: 'bg-gradient-to-r from-red-600 via-purple-600 to-indigo-600 text-white shadow-neon-fire' };
  if (streak >= 500) return { title: 'World Chaotic Origin Lifeform', bg: 'bg-purple-500/20 text-purple-300 border-purple-500/40' };
  if (streak >= 365) return { title: 'High Chaotic Origin Lifeform', bg: 'bg-pink-500/20 text-pink-300 border-pink-500/40' };
  if (streak >= 300) return { title: 'God Emperor', bg: 'bg-amber-500/20 text-amber-300 border-amber-500/40' };
  if (streak >= 250) return { title: 'God King', bg: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/40' };
  if (streak >= 210) return { title: 'Lord Of Chaos', bg: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' };
  if (streak >= 180) return { title: 'Eternal True God', bg: 'bg-blue-500/20 text-blue-300 border-blue-500/40' };
  if (streak >= 150) return { title: 'God Of Void', bg: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40' };
  if (streak >= 120) return { title: 'True God', bg: 'bg-teal-500/20 text-teal-300 border-teal-500/40' };
  if (streak >= 90) return { title: 'Universe Master', bg: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' };
  if (streak >= 75) return { title: 'Universe Venerable', bg: 'bg-green-500/20 text-green-300 border-green-500/40' };
  if (streak >= 60) return { title: 'Immortal / Undying', bg: 'bg-orange-500/20 text-orange-300 border-orange-500/40' };
  if (streak >= 45) return { title: 'World Master', bg: 'bg-amber-600/20 text-amber-400 border-amber-600/40' };
  if (streak >= 30) return { title: 'Domain Master', bg: 'bg-yellow-600/20 text-yellow-400 border-yellow-600/40' };
  if (streak >= 21) return { title: 'Universe', bg: 'bg-sky-500/20 text-sky-300 border-sky-500/40' };
  if (streak >= 14) return { title: 'Stellar', bg: 'bg-blue-600/20 text-blue-400 border-blue-600/40' };
  if (streak >= 7) return { title: 'Planetary / Star Traveler', bg: 'bg-cyan-600/20 text-cyan-400 border-cyan-600/40' };
  if (streak >= 3) return { title: 'Apprentice', bg: 'bg-slate-700 text-slate-300 border-slate-600' };
  return { title: 'Student', bg: 'bg-slate-800 text-slate-400 border-slate-700' };
}
...
function renderLobby() {
  ...
  habits.forEach(habit => {
    ...
    const realmInfo = getCultivationRealm(habit.currentStreak);
    ...
    card.innerHTML = `
      <div class="flex justify-between items-center mb-2.5">
        <div class="flex items-center space-x-2">
          <span class="text-[10px] font-black px-2 py-0.5 rounded-full border ${realmInfo.bg}">
            ⚡ ${realmInfo.title}
          </span>
          ${riskBadge}
        </div>
...
function renderDetailContent(id) {
  ...
  badge.className = `px-3 py-1 rounded-full text-xs font-black ${realmInfo.bg}`;
  badge.innerText = `⚡ ${realmInfo.title}`;
  ...
  if (isCompletedToday) {
    ...
    actionBox.innerHTML = `
      <div class="grid grid-cols-4 gap-2">
        ...
        <button onclick="restHabit('${habit.id}')" title="Rest Day (Streak Kekal)" class="col-span-1 py-3 rounded-xl font-extrabold text-xs bg-slate-800 text-cyan-400 border border-slate-700 hover:bg-cyan-950/40 active:scale-95 transition-all flex items-center justify-center">
          <i data-lucide="moon" class="w-4 h-4"></i>
        </button>
      </div>
    `;
...
function confirmResetStreakOnlyFromDetail() {
  ...
  document.getElementById('confirmMessage').innerText = `Current streak untuk "${habit.name}" akan kembali ke 0 manakala Highest Streak (${habit.highestStreak} hari) dan Total Completed (${habit.totalCompleted} hari) kekal terpelihara.`;
