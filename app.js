...
function renderMonthlyCalendar(habit) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    cell.onclick = () => {
      ...
      const confirmModalMsg = document.getElementById('confirmModal');
      document.getElementById('confirmTitle').innerText = `Info Tarikh: ${dateKey}`;
      document.getElementById('confirmMessage').innerText = msg;
      document.getElementById('confirmBtnAction').className = 'hidden';
      confirmModalMsg.classList.remove('hidden');
      setTimeout(() => confirmModalMsg.classList.remove('opacity-0'), 10);
    };
